			...
			if(queryObject.has("catalog_id") || queryObject.has("feed_name") || queryObject.has("access_token") || queryObject.has("country") || queryObject.has("currency") || queryObject.has("hostname")){
				...
				if(null != requestObj){
					...
					if(requestObj.has("success")){
						...
						out.println(new JSONObject().put("success", false).put("feed_id", "NA").put("message", "SOMETHING WENT WRONG (BAD RESPONSE). PLEASE TRY AGAIN."));
					}
					
				}
				else{
					out.println(new JSONObject().put("success", false).put("feed_id", "N/A").put("message", "SOMETHING WENT WRONG (BAD RESPONSE). PLEASE TRY AGAIN."));
				}
				
			}
			else{
				out.println(new JSONObject().put("success", false).put("feed_id", "NA").put("message", "ONE OR MORE MANDATORY FIELDS ARE MISSING. PLEASE VERIFY YOUR REQUEST AGAIN."));
			}
