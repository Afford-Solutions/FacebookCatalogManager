			...
			if(queryObject.has("business_id") || queryObject.has("catalog_name") || queryObject.has("access_token")){
				...
				if(null != requestObj){
					...
					if(requestObj.has("success")){
						...
						out.println(new JSONObject().put("success", false).put("catalog_id", "NA").put("message", "SOMETHING WENT WRONG (BAD RESPONSE). PLEASE TRY AGAIN."));
					}
					
				}
				else{
					out.println(new JSONObject().put("success", false).put("catalog_id", "N/A").put("message", "SOMETHING WENT WRONG (BAD RESPONSE). PLEASE TRY AGAIN."));
				}
				
			}
			else{
				out.println(new JSONObject().put("success", false).put("catalog_id", "NA").put("message", "ONE OR MORE MANDATORY FIELDS ARE MISSING. PLEASE VERIFY YOUR REQUEST AGAIN."));
			}
